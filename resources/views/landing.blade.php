...
<head>
    ...
    <title>SPX Achievement - Week {{ $weekNumber }}</title>
...
<body class="bg-gray-50 min-h-screen">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div class="flex items-center justify-between">
                ...
                <div class="text-right flex items-center gap-4">
                    <div>
                        <p class="text-sm text-gray-500">Minggu {{ $weekNumber }}</p>
                        <p class="text-xs text-gray-400">{{ $weekStart->format('d M') }} - {{ $weekEnd->format('d M Y') }}</p>
                    </div>
            ...
            <div class="flex gap-2 justify-center">
                @foreach ($weekDays as $day)
                    <div class="flex-1 text-center px-2 py-2 rounded-lg {{ $day['is_today'] ? 'border-2 font-bold' : 'border border-gray-200' }}"
                        ...
                        <div class="text-sm {{ $day['is_today'] ? 'font-bold' : 'font-medium' }}" style="{{ $day['is_today'] ? 'color: #EE4D2D;' : '' }}">
                            Day {{ $day['day_number'] }}
                        </div>
                        <div class="text-xs text-gray-400">{{ $day['date_display'] }}</div>
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="bg-white rounded-lg shadow-sm p-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-xs text-gray-400">Minggu ke-{{ $weekNumber }}</p>
                        <p class="text-sm font-semibold text-gray-800">{{ $weekStart->format('d M') }} - {{ $weekEnd->format('d M Y') }}</p>
                    </div>
                </div>
            </div>
    ...
    {{-- Manpower List --}}
    <script type="application/json" id="initial-data">{!! json_encode($data) !!}</script>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-20"
        ...
        {{-- Load More Sentinel (for IntersectionObserver auto-load) --}}
        ...
        function manpowerList() {
            return {
                ...
                init() {
                    ...
                    this.hasMore = {{ $hasMore ? 'true' : 'false' }};
