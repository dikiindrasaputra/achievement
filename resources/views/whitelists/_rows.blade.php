            ...
            <td class="px-4 py-3 text-center {{ $day['is_today'] ? 'bg-indigo-50 dark:bg-indigo-900/30' : '' }}">
                @if ($isWhitelisted)
                    <button type="button"
                        ...
                        class="w-8 h-8 rounded-full {{ $whitelistRecord->reason === 'Izin' ? 'bg-blue-500' : 'bg-green-500' }} text-white hover:bg-red-500 transition-colors inline-flex items-center justify-center cursor-pointer" title="{{ $whitelistRecord->reason ?? 'Libur' }} — Klik untuk hapus">
                        <span class="text-[9px] font-bold">{{ $whitelistRecord->reason === 'Izin' ? 'IZN' : 'LBR' }}</span>
                    </button>
                @elseif ($day['is_past'])
                ...
                @elseif ($remainingQuota > 0 || true)
                    <div class="relative" x-data="{ open: false }">
                        <button type="button" @click="open = !open" class="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-600 hover:bg-green-500 hover:text-white transition-colors inline-flex items-center justify-center" title="Klik untuk whitelist">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                        </button>
                        <div x-show="open" @click.away="open = false" x-cloak
                             class="absolute left-1/2 -translate-x-1/2 mt-1 z-20 bg-white rounded-lg shadow-lg border border-gray-200 py-1 w-28">
                            <form action="{{ route('whitelists.store') }}" method="POST">
                                @csrf
                                <input type="hidden" name="manpower_id" value="{{ $person->id }}">
                                <input type="hidden" name="date" value="{{ $day['date'] }}">
                                <input type="hidden" name="reason" value="Libur">
                                <button type="submit" class="w-full text-left px-3 py-1.5 text-sm hover:bg-green-50 text-gray-700" @click="open = false">Libur</button>
                            </form>
                            <form action="{{ route('whitelists.store') }}" method="POST">
                                @csrf
                                <input type="hidden" name="manpower_id" value="{{ $person->id }}">
                                <input type="hidden" name="date" value="{{ $day['date'] }}">
                                <input type="hidden" name="reason" value="Izin">
                                <button type="submit" class="w-full text-left px-3 py-1.5 text-sm hover:bg-blue-50 text-gray-700" @click="open = false">Izin</button>
                            </form>
                        </div>
                    </div>
                @else
